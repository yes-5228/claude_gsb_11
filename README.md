...
面向城市公厕管养单位的巡查记录与整改闭环管理系统，覆盖 **公厕台账 → 保洁巡查 → 巡查路线 → 问题上报 → 整改跟踪** 五条业务主线。后端为 FastAPI + SQLAlchemy，前端为 React + Vite，前后端均按模块拆分，可单独开发、单独部署。
...
| 巡查路线 | `/patrols` | 计划路线维护（有序点位 + 计划停留时长）、巡查起止时间与逐点位到离采集、到位率与偏离自动比对、明显偏离强制补充说明、计划 vs 实际轨迹对比 |
...
│   │   ├── models               # ORM 模型：公厕、巡查、计划路线、巡查路线记录、问题、整改流水
...
| GET | `/patrol-plans` | 计划路线查询（district/inspector/关键字） |
| POST | `/patrol-plans` | 新增计划路线（有序点位 + 计划停留时长） |
| GET/PATCH/DELETE | `/patrol-plans/{id}` | 详情 / 更新 / 删除；有巡查记录时返回 409，`force=true` 级联删除 |
| GET | `/patrol-records` | 巡查路线记录查询（plan_id/inspector/偏离程度/日期区间/关键字） |
| POST | `/patrol-records` | 新增巡查路线记录，服务端自动计算停留时长、到位率与偏离情况 |
| GET/PATCH/DELETE | `/patrol-records/{id}` | 详情（含计划 vs 实际比对）/ 更新（可补充偏离说明）/ 删除 |
...
- **路线可追溯**：每条巡查路线记录采集开始/结束时间与逐点位到离时间，自动计算各点位停留时长；轨迹点位须位于巡查起止时间内且不可重复。比对结果（到位率、偏离程度、偏离明细）在写入时快照保存，事后修改计划路线不影响历史记录。
- **到位率与偏离判定**：到位率 = 实际到访的计划点位数 / 计划点位总数 × 100%。无漏巡、无计划外点位且顺序一致为「无偏离」；到位率低于 70% 或漏巡点位达到 2 个为「明显偏离」；其余（漏巡 1 个点、出现计划外点位、顺序不一致）为「轻微偏离」。
- **偏离说明**：判定为「明显偏离」的巡查必须填写偏离补充说明，否则接口返回 400；已保存的明显偏离记录不允许清空说明，可在详情弹窗中随时补充/修订。
...
`SEED_ON_STARTUP=true`（默认）且数据库为空时，会自动写入：10 座公厕（4 个区域、三类等级、含维修/停用状态）、近 14 天约 90 条巡查记录、3 条计划路线与近 7 天 15 条巡查路线记录（覆盖无偏离/轻微偏离/明显偏离，明显偏离均带补充说明）、13 条不同整改阶段的问题及其完整整改轨迹。数据由固定随机种子生成，结果可复现；如需重置，删除 `backend/data/app.db`（或 `docker compose down -v`）后重启即可。

## 测试与验证

```bash
cd backend && pytest -q          # 接口测试（覆盖台账 CRUD、删除保护、评分、路线比对与偏离说明约束、流程流转、统计）
cd frontend && npm run build     # 生产构建
```

本项目完成时已实际运行验证：

- 后端 `pytest`：13 个用例全部通过；`/health`、台账/巡查/路线/问题/统计/字典接口均返回预期数据。
